...
export function getClosestString(input: string, candidates: readonly string[], maxDist = 3): string | undefined {
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        const dist = levenshtein(input, norm(candidates[i]));

        if (dist < bestDist) {
            bestDist = dist;
            bestStr = candidates[i];
        }
    }

    return bestDist > maxDist ? undefined : bestStr;
}
